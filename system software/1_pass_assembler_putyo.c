...
int main()
{
    ...
    FILE *input, *output,*fp1,*fp2, *length_file;
    char line[1000],mnemonic[1000];
    int start;
    input = fopen("input_putyo.txt", "r");
    fp1 = fopen("optab_putyo.txt", "r");
    fp2 = fopen("symtab_putyo.txt", "w");
    output = fopen("output_putyo.txt", "w");
    length_file = fopen("length_inter.txt","w");
