...
{
    int result;
    result = sscanf(line, "%s %s %s %s",token->address, token->label, token->opcode, token->operands);
    if (result == 3)
    {
...
int main() {
    ...
    char line[1000];
    int start;
    input = fopen("avr_output_1.txt", "r");
...
while(fgets(line, 1000, input) != NULL)
   ...
   unsigned long objCode = objectCode(token.opcode, token.operands);
    printf("0x%lX\n", objCode);
