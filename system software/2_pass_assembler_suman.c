...
int main()
{
    struct Token token; //structure for input file
    struct Token token1;
    ...
    while (fscanf(fp2, "%s %x", sym[Line].label, &sym[Line].address) != EOF) //reading symbol table content to array of structure
        Line++;
    rewind(fp2);
    ...
    rewind(input_pass1);//rewind input file pointer to start of file
    //at this point token stores the last line of input file
